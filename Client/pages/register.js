...
export default function Register() {
    ...
    const handleSubmit = () => {
        ...
        petition_post("Register", { data: form })
            .then((result) => {
                localStorage.setItem("userAuth", JSON.stringify(result.data.data))
                window.location.href = "/dashboard"
            })
            .catch((err) => { window.alert(err.response?.data.data) || "error in register" })
    ...
    return <div className="  w-[30rem] mx-auto bg-[#dbdada] rounded-md p-5 mt-10">
        ...
        <div className="text-center pt-7">
            <button onClick={handleSubmit} className="rounded-md bg-[white] font-bold border-2 h-10 border-[#424141]  w-full mx-auto ">  Register </button>
